distGetData <- function(ID){
  ...
  core.tbl <- tbl(KELuser, "core") %>%
    ...
    select(sp_group_dist, sp_type, tree_id, dbh_mm, core_id = id, missing_mm, missing_years) %>%
    collect()
  ...
  dist_param.tbl <- read_csv("Data/balk_dist_param.csv")
...
growthCalculate <- function(data.in, windowLength = 10){
  ...
  growth.tbl <- inner_join(data.in$core, data.in$ring, by = "core_id") %>%
    ...
    mutate(#missing_mm = ifelse(is.na(missing_mm), 0, missing_mm),
           ...
           ai = fg - pg) %>%
    ungroup() %>%
    select(sp_type, sp_group_dist, tree_id, core_id, ring_id, year, incr_mm, age, dbh_mm = dbh_growth, pg, fg, ai)
